...
### 2.3 协议层安全
- **元数据加密**: 采用 `AES-128-GCM` 对目标地址进行加密。
- **密钥派生**: 每个流的加密密钥通过 HKDF 从 PSK 和从头部读取的 **StreamID** 共同派生，确保密钥的唯一性与抗重放能力。
- **显式同步**: StreamID 明文传输于 QUIC 加密隧道内部的协议头中，消除了客户端与服务端因并发导致的密钥不同步问题。
