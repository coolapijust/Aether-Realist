...
func (h *sentPacketHandler) SetCongestionControl(c congestion.SendAlgorithm) {
	if cd, ok := c.(congestion.SendAlgorithmWithDebugInfos); ok {
		h.congestion = cd
	} else {
		// Wrap it if it doesn't have debug infos
		h.congestion = &congestionWrapper{SendAlgorithm: c}
	}
}

type congestionWrapper struct {
	congestion.SendAlgorithm
}

func (w *congestionWrapper) InSlowStart() bool            { return false }
func (w *congestionWrapper) InRecovery() bool             { return false }
func (w *congestionWrapper) GetCongestionWindow() protocol.ByteCount {
	return w.SendAlgorithm.GetCongestionWindow()
}
...
func (h *sentPacketHandler) ReceivedAck(ack *wire.AckFrame, encLevel protocol.EncryptionLevel, rcvTime monotime.Time) (bool /* contained 1-RTT packet */, error) {
	...
	newlyLost := h.detectLostPackets(rcvTime, encLevel)
	if encLevel == protocol.Encryption1RTT {
		h.detectLostPathProbes(rcvTime)
	}
	var newlyAcked []congestion.AckedPacketInfo
	for _, p := range ackedPackets {
		if p.includedInBytesInFlight && !p.declaredLost {
			h.congestion.OnPacketAcked(p.PacketNumber, p.Length, priorInFlight, rcvTime)
			newlyAcked = append(newlyAcked, congestion.AckedPacketInfo{
	...
	if len(newlyAcked) > 0 || len(newlyLost) > 0 {
		h.congestion.OnCongestionEventEx(priorInFlight, rcvTime, newlyAcked, newlyLost)
	}
...
func (h *sentPacketHandler) detectLostPackets(now monotime.Time, encLevel protocol.EncryptionLevel) []congestion.LostPacketInfo {
	pnSpace := h.getPacketNumberSpace(encLevel)
	pnSpace.lossTime = 0
	...
	priorInFlight := h.bytesInFlight
	var newlyLost []congestion.LostPacketInfo
	for pn, p := range pnSpace.history.Packets() {
		...
		if packetLost {
			...
			if !p.isPathProbePacket && p.IsAckEliciting() {
				...
				if !p.IsPathMTUProbePacket {
					...
					newlyLost = append(newlyLost, congestion.LostPacketInfo{
						PacketNumber: pn,
						LostBytes:    p.Length,
					})
				}
