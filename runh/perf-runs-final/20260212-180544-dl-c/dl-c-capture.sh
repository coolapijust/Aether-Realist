...
set -euo pipefail
sleep 5
timeout 60 docker compose -f "/root/deploy/docker-compose.yml" logs --since "2026-02-12T10:05:49Z" -f aether-gateway-core 2>&1 | grep -E --line-buffered "\[PERF(-GW)?\]" > "/root/perf-runs/20260212-180544-dl-c/dl-c-perf.log" || true

{
  echo "preset=dl-c"
  echo "since_utc=2026-02-12T10:05:49Z"
  echo "window_sec=60"
  if [[ -s "/root/perf-runs/20260212-180544-dl-c/dl-c-perf.log" ]]; then
    awk '
    ...
    ' "/root/perf-runs/20260212-180544-dl-c/dl-c-perf.log"
    echo "--- tail ---"
    tail -n 5 "/root/perf-runs/20260212-180544-dl-c/dl-c-perf.log" || true
  else
    echo "points=0"
    echo "note=perf file is empty; check runner log"
  fi
} > "/root/perf-runs/20260212-180544-dl-c/dl-c-summary.log" 2>&1
