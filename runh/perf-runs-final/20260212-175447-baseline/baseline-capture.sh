...
set -euo pipefail
sleep 5
timeout 60 docker compose -f "/root/deploy/docker-compose.yml" logs --since "2026-02-12T09:55:25Z" -f aether-gateway-core 2>&1 | grep -E --line-buffered "\[PERF(-GW)?\]" > "/root/perf-runs/20260212-175447-baseline/baseline-perf.log" || true

{
  echo "preset=baseline"
  echo "since_utc=2026-02-12T09:55:25Z"
  echo "window_sec=60"
  if [[ -s "/root/perf-runs/20260212-175447-baseline/baseline-perf.log" ]]; then
    awk '
    ...
    ' "/root/perf-runs/20260212-175447-baseline/baseline-perf.log"
    echo "--- tail ---"
    tail -n 5 "/root/perf-runs/20260212-175447-baseline/baseline-perf.log" || true
  else
    echo "points=0"
    echo "note=perf file is empty; check runner log"
  fi
} > "/root/perf-runs/20260212-175447-baseline/baseline-summary.log" 2>&1
